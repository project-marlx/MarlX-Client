...
const ACTION_DISKINFO_UPDATE = 2
...
const ACTION_RESPOND_FILE_CONTENT = 4
...
const ACTION_UPDATE_DISKINFO = 202
...
func (fih *FileInfoHeader) String() string {
	return fmt.Sprintf("FileInfoHeader{FragCount: %d, IsDir: %t, Name: %s, Size: %d, UniqueId: %x, UserToken: %x}",
		fih.FragCount, fih.IsDir, fih.Name, fih.Size, fih.UniqueId, fih.UserToken)
}
